...
def is_valid_property_start(data: bytes, pos: int, names: list) -> bool:
    """Check if the given position looks like a valid property tag start."""
    from ue2.reader import read_compact_index_at

    if pos >= len(data):
        return False
    if data[pos] == 0:  # 'None' terminator
        return True

    try:
        # 1. Must be a valid NameIndex
        name_idx, next_pos = read_compact_index_at(data, pos)
        if name_idx < 0 or name_idx >= len(names):
            return False

        # 2. Must have an InfoByte
        if next_pos >= len(data):
            return False
        info = data[next_pos]

        # 3. InfoByte must have valid Type (1-15) and SizeType (0-7)
        prop_type = info & 0x0F
        size_type = (info >> 4) & 0x07
        if not (1 <= prop_type <= 15):
            return False
...
def find_none_terminator(data: bytes, names: list) -> int:
    ...
    while pos < max_pos:
        try:
            ...
            raise ValueError(
                "Unexpected end of data or invalid compact index in properties"
            )
        ...
        if name_idx < 0 or name_idx >= len(names):
            raise ValueError(f"Invalid name index {name_idx} at pos {pos-1}")
        ...
        # 1. Optional struct name for StructProperty (BEFORE size)
        ...
        # 3. Optional array index (AFTER size)
        array_index_size = 0
        if is_array and prop_type != 3:
            # Array index is a compact index
            # Vanguard uses a slightly different compact index for array indices
            if pos < len(data):
                b = data[pos]
                array_index_size = 1
                if b >= 128:
        ...
        # 4. Adaptive skip logic
        # Some Vanguard files use "Inclusive" size (size field includes itself)
        # others use "Exclusive" size (standard UE2).
        if size_type >= 5:
            exclusive_pos = pos + array_index_size + size
            inclusive_pos = pos + array_index_size + size - size_bytes

            # Probe both offsets to see which looks like a valid next property
            inc_valid = is_valid_property_start(data, inclusive_pos, names)
            exc_valid = is_valid_property_start(data, exclusive_pos, names)

            if inc_valid and exc_valid:
                # Both look valid - resolve conflict
                # 1. Prefer a non-None property start over a None terminator
                if data[inclusive_pos] != 0 and data[exclusive_pos] == 0:
                    pos = inclusive_pos
                elif data[exclusive_pos] != 0 and data[inclusive_pos] == 0:
                    pos = exclusive_pos
                else:
                    # Default to exclusive if both are None or both are properties
                    pos = exclusive_pos
            elif inc_valid:
                pos = inclusive_pos
            else:
                # Fallback to exclusive even if not valid (best effort)
                pos = exclusive_pos
        else:
            pos = pos + array_index_size + size

    raise ValueError(f"Never found None terminator or exceeded limit (pos={pos})")
...
def parse_staticmesh(data: bytes, names: list, serial_offset: int) -> dict:
    ...
    core_start = -1
    detected_v_off = -1  # Track the offset where we found the anchor
    v_anchors = [60482, 60484, 60485, 236, 11, 12, 128, 129]  # Known versions
    ...
    if core_start == -1:
        ...
        # We search for the pattern [Bounds][Padding?][Version][SectionCount]
        # Vanguard can have extra padding/fields (4-8 bytes) between properties and bounds,
        # or between bounds and InternalVersion.
        search_limit = min(len(data) - 60, 2048)
        
        # Two-pass search: First look for reasonable sec_count (< 20), then fallback to < 200
        for max_sc in [20, 200]:
            if core_start != -1:
                break
            for i in range(0, search_limit):
                # InternalVersion anchor can be at various offsets after prop_end
                # standard: 40/41, vanguard-variant: 44/45, 48/52
                for v_off in [40, 41, 44, 45, 48, 52]:
                    if i + v_off + 8 <= len(data):
                        ver = struct.unpack("<i", data[i + v_off : i + v_off + 4])[0]
                        if ver in v_anchors:
                            sc = struct.unpack("<i", data[i + v_off + 4 : i + v_off + 8])[0]
                            if 0 <= sc < max_sc:
                                core_start = i
                                detected_v_off = v_off
    ...
    if core_start == -1:
        ...
        result["error_message"] = (
            "Could not find StaticMesh core anchor (InternalVersion)"
        )
        return result

    # Use the detected offset (fallback to 40 if somehow not set)
    v_off = detected_v_off if detected_v_off != -1 else 40

    internal_version = struct.unpack(
        "<i", data[core_start + v_off : core_start + v_off + 4]
    )[0]
    sec_count = struct.unpack(
        "<i", data[core_start + v_off + 4 : core_start + v_off + 8]
    )[0]
    ...
    # Start search after minimum bounds (prop_end + 40 is earliest possible)
    # But don't start before core_start
    search_limit = len(data) - 10
    search_start = max(core_start + 40, prop_end + 40)


    current_search = search_start
    while current_search < search_limit:
        # Fast C-level search
        match_pos = data.find(b"\x00\x00\x00\x00\x00\x00", current_search)
        if match_pos == -1 or match_pos >= search_limit:
            break

        # Check the pointer immediately following the zeros
        ptr = struct.unpack("<I", data[match_pos + 6 : match_pos + 10])[0]

        # Must be a valid absolute pointer within the file
        # AND must be pointing to a position AFTER the current padding
        if (
            padding_pos + 10 + serial_offset
            < ptr
            <= serial_offset + len(data)
        ):
            padding_pos = match_pos
            skip_pos = ptr
            # Found a valid one; normally we take the first valid one we find
            break
        elif ptr == 0 and padding_pos == -1:
            # Potential fallback if no valid pointer is ever found (empty RawTriangles)
            # We only look for 10 zeros if we haven't found a better match
            if data[match_pos : match_pos + 10] == b"\x00" * 10:
                padding_pos = match_pos
                skip_pos = 0

        # Move search forward
        current_search = match_pos + 1

    if padding_pos == -1:
        result["parse_status"] = "error"
        result["error_message"] = (
            "Could not find RawTriangles sync padding with valid skip_pos"
        )
        return result
    ...
    # Position after core section (before potential RawTriangles payload)
    core_end = core_start + core_bytes
    ...
    # Bytes between core end and skip target are RawTriangles payload
    raw_tri_size = relative_skip - core_end if relative_skip > core_end else 0
    if raw_tri_size > 0:
        result["data"]["raw_triangles_payload_size"] = raw_tri_size
        # Note: We don't add raw_tri_size to bytes_parsed because we're skipping over it
    ...
    # Handle unsupported variants or empty LODs
    if lod_count <= 0:
        result["parse_status"] = "skipped"
        result["error_message"] = f"No LODs found (lod_count={lod_count}, lod_version={lod_version})"
        return result
    
    if lod_version >= 2:
        result["parse_status"] = "skipped"  
        result["error_message"] = f"Unsupported LOD version {lod_version} (uses populated standard streams)"
        return result
    ...
    for i in range(lod_count):
        ...
        if result["bytes_parsed"] + 16 > len(data):
            # Not enough data for another LOD - break with what we have
            result["uses_heuristics"] = True
            break
        
        h12 = struct.unpack(
            "<III", data[result["bytes_parsed"] : result["bytes_parsed"] + 12]
        )
        v12 = h12[2]

        h16 = struct.unpack(
            "<IIII", data[result["bytes_parsed"] : result["bytes_parsed"] + 16]
        )
        v16 = h16[3]
        ...
        if v12 > 100000 or v12 == 0:
            if 0 < v16 <= 100000:
                ...
                # Both headers give absurd vertex counts - likely reached end of valid LOD data
                # Break with what we have
                break
        
        # Verify we actually have space for this many vertices
        remaining_bytes = len(data) - result["bytes_parsed"] - header_size
        required_bytes = vertex_count * 56  # 56 bytes per vertex
        if required_bytes > remaining_bytes:
            # Not enough data for claimed vertices - break with what we have
            break
        else:
        ...
        # Vertices (56 bytes each) - use direct data access for consistent tracking
        vertex_bytes = lod_data["vertex_count"] * 56
        vertices_data = data[result["bytes_parsed"] : result["bytes_parsed"] + vertex_bytes]
            ...
            if v_offset + 12 <= len(vertices_data):
                ...
                if v_offset + 24 <= len(vertices_data):
                    nx, ny, nz = struct.unpack(
                        "<fff", vertices_data[v_offset + 12 : v_offset + 24]
                    )
                # Also read UV (next 8 bytes)
                u, v = 0.0, 0.0
                if v_offset + 32 <= len(vertices_data):
                    u, v = struct.unpack(
                        "<ff", vertices_data[v_offset + 24 : v_offset + 32]
                    )
        ...
        for offset in range(0, 64, 4):
            ...
            if probe_pos + 24 <= len(data):
                ...
                if 3 <= ic <= 1000000:
                    ...
                    first_idx = struct.unpack(
                        "<H", data[probe_pos + 24 : probe_pos + 26]
                    )[0]
        ...
        if index_count == 0:
            ...
            if pv_search_start + 24 <= len(data):
                index_count = struct.unpack(
                    "<I", data[pv_search_start + 20 : pv_search_start + 24]
                )[0]
        ...
        indices_raw = data[
            result["bytes_parsed"] : result["bytes_parsed"] + index_count * 2
        ]
...
if __name__ == "__main__":
    ...
    test_files = [
        ...
        os.path.join(config.ASSETS_PATH, "Meshes", "W2_Agua_mesh.usx"),
        os.path.join(config.ASSETS_PATH, "Meshes", "Ra1_P1_C1_Test_Mesh.usx"),
        os.path.join(config.ASSETS_PATH, "Meshes", "Ra44_P1_C3_Racks_Mesh.usx"),
        os.path.join(config.ASSETS_PATH, "Meshes", "Ra7000_P1_Weapons_mesh.usx"),
        os.path.join(config.ASSETS_PATH, "Meshes", "Ra21_P1_C2_Armor_meshes.usx"),
    ]
        ...
        try:
            ...
            if result["data"].get("lods"):
                for i, lod in enumerate(result["data"]["lods"]):
                    print(
                        f"  LOD {i}: {lod['vertex_count']} vertices, {lod['index_count']} indices"
                    )
                    if lod["indices"]:
                        triangles = len(lod["indices"]) // 3
                        print(
                            f"         {triangles} triangles: {lod['indices'][:12]}..."
                        )

            if result["unknown_regions"]:
                for region in result["unknown_regions"]:
                    print(
                        f"  Unknown region: {region['context']} ({region['size']} bytes)"
                    )
