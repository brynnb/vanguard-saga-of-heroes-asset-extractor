    ...
    while offset < len(data) - 1 and len(result) < max_props:
        ...
        if array_flag and prop_type != 3:
            if offset < len(data):
                b = data[offset]
                offset += 1
                if b >= 128 and offset < len(data):
                    b2 = data[offset]
                    offset += 1
                    if (b & 0x40) and offset + 1 < len(data):
                        offset += 2
...
def score_property_chain(data: bytes, names: list, start_offset: int) -> int:
    ...
    while offset < len(data) - 1 and seen_props < 100:
        ...
        # Validate property name vs type (must match parse_properties checks)
        if prop_type in (7, 13) and prop_name in NON_STRING_PROP_NAMES:
            break
        if prop_type == 6 and prop_name in NON_NAME_PROP_NAMES:
            break
        if prop_type == 9 and prop_name in NON_ARRAY_PROP_NAMES:
            break
        ...
        if prop_type == 10:
            ...
            if struct_idx is not None and 0 <= struct_idx < len(names):
                if names[struct_idx] in INVALID_STRUCT_NAMES:
                    break
...
def parse_properties(data: bytes, names: list, start_offset: int) -> list:
    ...
    while offset < len(data) - 1 and len(properties) < 200:
        ...
        if prop_name == "None":
            # Vanguard specific: TerrainInfo has several empty property lists 
            # before the real one. Keep searching until end of data.
            offset = new_offset
            current_list += 1
            consecutive_nones += 1
            continue
        ...
        if prop_type == 10:
            ...
            if struct_idx is not None and 0 <= struct_idx < len(names):
                struct_name = names[struct_idx]
                # Validate struct name - reject known invalid names
        ...
        if array_flag and prop_type != 3:
            if offset < len(data):
                ...
                if b < 128:
                    ...
                    offset += 1
                    if not (b & 0x40):
                        array_index = ((b << 8) | b2) & 0x3FFF
                    elif offset + 1 < len(data):
                        b3 = data[offset]
                        b4 = data[offset + 1]
                        offset += 2
                        array_index = ((b << 24) | (b2 << 16) | (b3 << 8) | b4) & 0x3FFFFF
