...
DEFAULT_UNREAL_LIBRARY_DIR = config.PROJECT_ROOT / "external" / "Unreal-Library"
...
def cmd_setup(args: argparse.Namespace) -> None:
    env = build_env(args)
    command = [sys.executable, "-m", "scripts.setup_assets"]
    for attr, flag in [
        ("reset", "--reset"),
        ("full", "--full"),
        ("skip_core", "--skip-core"),
...
def cmd_build_shaders(args: argparse.Namespace) -> None:
    ...
    run(
        ...
        [
            sys.executable,
            "-m", "scripts.extractors.build_shader_texture_map",
...
def cmd_extract_terrain(args: argparse.Namespace) -> None:
    env = build_env(args)
    command = [sys.executable, "-m", "scripts.extractors.extract_all_terrain"]
    if args.chunk:
        command.extend(["--chunk", args.chunk])
    else:
        command.append("--all")
...
def cmd_export_meshes(args: argparse.Namespace) -> None:
    env = build_env(args)
    command = [sys.executable, "-m", "scripts.extractors.staticmesh_pipeline"]
    if args.file:
...
def cmd_export_characters(args: argparse.Namespace) -> None:
    env = build_env(args)
    command = [sys.executable, "-m", "scripts.exporters.export_character_meshes"]
    if args.filter:
        command.extend(["--filter", args.filter])
    run("Export character meshes", command, env)
    run("Build authoritative item appearance catalog", [sys.executable, "-m", "scripts.extractors.decode_items"], env)
    run(
        "Decode authored attachment groups",
        [sys.executable, "-m", "scripts.extractors.decode_attachment_groups"],
        env,
    )
    run("Generate playable race data", [sys.executable, "-m", "scripts.generators.generate_playable_races"], env)
    ...
    if customization_file.exists():
        run(
            ...
            [
                sys.executable,
                "-m", "scripts.generators.generate_customization_data",
                "--emu-root",
                env["VANGUARD_EMU_PATH"],
            ],
            env,
...
def cmd_export_animations(args: argparse.Namespace) -> None:
    ...
    run("Export UE2 skeletal animations", [sys.executable, "-m", "scripts.exporters.export_animations"], env)
...
def cmd_extract_world(args: argparse.Namespace) -> None:
    ...
    run("Split SGO actors by class", [sys.executable, "-m", "scripts.extractors.split_sgo_by_class"], env)
    run("Fold SGO extras into prefabs", [sys.executable, "-m", "scripts.extractors.fold_actors_into_prefabs"], env)
    run("Extract particle texture refs", [sys.executable, "-m", "scripts.extractors.extract_particle_textures"], env)
    run(
    ...
    if args.generate_objects:
        object_command = [sys.executable, "-m", "scripts.generators.generate_objects_from_txt", "--all"]
        if args.object_workers != 1:
