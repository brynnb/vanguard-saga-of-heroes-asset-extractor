"""Runtime path configuration for the Vanguard extraction package.
...
Override paths with environment variables when needed:
    ...
    VANGUARD_WORKSPACE_ROOT=/path/for/generated/output
"""
...
# Installed commands must never write generated data into site-packages.  The
# workspace defaults to the caller's current directory and can be pinned for
# subprocesses with VANGUARD_WORKSPACE_ROOT.
PROJECT_ROOT = Path(
    os.environ.get("VANGUARD_WORKSPACE_ROOT", str(Path.cwd()))
).expanduser().resolve()
RENDERER_ROOT = str(PROJECT_ROOT)
