...
# Add paths - need to go up one level from extractors to renderer  
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
RENDERER_DIR = os.path.dirname(SCRIPT_DIR)  # Go up from extractors to renderer
sys.path.insert(0, os.path.join(RENDERER_DIR, 'lib'))
...
def get_or_create_file_id(conn: sqlite3.Connection, pkg_path: str) -> int:
    ...
    # Check if exists
    cursor.execute("SELECT id FROM files WHERE file_path = ?", (pkg_path,))
    row = cursor.fetchone()
    if row:
        return row[0]
    ...
    size = os.path.getsize(pkg_path) if os.path.exists(pkg_path) else 0
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    """, (pkg_path, filename, size, parent, ext, 'mesh'))
...
def run_pipeline(file_pattern: str = None, export_gltf: bool = True, export_only: bool = False, limit: int = 0):
    """
    ...
    for i, pkg_path in enumerate(files):
        print(f"[{i+1}/{len(files)}] Processing {os.path.basename(pkg_path)}...")
